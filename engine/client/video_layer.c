...
void vlEndDraw(PLDraw *draw) {
    ...
    glBufferData(GL_ARRAY_BUFFER, draw->numverts * sizeof(plVector3f_t) * sizeof(PLVertex), draw->vertices->position,
                 GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, draw->_gl_vbo[_VL_BUFFER_TEXCOORDS]);
    glBufferData(GL_ARRAY_BUFFER, draw->numverts * sizeof(plVector2f_t) * sizeof(PLVertex), draw->vertices->ST,
                 GL_DYNAMIC_DRAW);
