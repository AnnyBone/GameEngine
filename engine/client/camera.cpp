...
extern "C" void CameraManager_SetParentEntity(EngineCamera *camera, ClientEntity_t *parent) {
    if (!camera) {
        Sys_Error("Received invalid camera when attempting to set parent entity!\n");
        return;
    } // Keep compiler happy...
...
extern "C" void CameraManager_SetViewEntity(EngineCamera *camera, ClientEntity_t *child) {
    if (!camera) {
        Sys_Error("Received invalid camera when attempting to set view entity!\n");
        return;
    } // Keep compiler happy...
...
extern "C" ClientEntity_t *CameraManager_GetParentEntity(EngineCamera *camera) {
    if (!camera) {
        Sys_Error("Received invalid camera when attempting to get parent entity!\n");
        return NULL;
    } // Keep compiler happy...
...
extern "C" ClientEntity_t *CameraManager_GetViewEntity(EngineCamera *camera) {
    if (!camera) {
        Sys_Error("Received invalid camera when attempting to get view entity!\n");
        return NULL;
    } // Keep compiler happy...
    return camera->GetParentEntity();
}
    ...
    if (cl.worldmodel) {
        ...
        if (r_waterwarp.value) {
            ...
            if (contents == BSP_CONTENTS_WATER || contents == BSP_CONTENTS_SLIME || contents == BSP_CONTENTS_LAVA) {
                // variance is a percentage of width, where width = 2 * tan(fov / 2) otherwise the effect is too
                // dramatic at high FOV and too subtle at low FOV.  what a mess!
                fovxx = (float) (std::atan(
                        std::tan(PL_DEG2RAD(_fovx) / 2) * (0.97f + std::sin((float) cl.time * 1.5f) * 0.03f)) * 2.0f /
                                 PL_PI_DIV180);
                fovyy = (float) (std::atan(
                        std::tan(PL_DEG2RAD(_fovy) / 2) * (1.03f - std::sin((float) cl.time * 1.5f) * 0.03f)) * 2.0f /
                                 PL_PI_DIV180);
            }
        }
...
void Camera::SimulateBob() {
    ...
    // Forward cycle
    cycle[0] = ((float) cl.time - (int) (cl.time / cv_camera_forwardcycle.value) * cv_camera_forwardcycle.value) /
               cv_camera_forwardcycle.value;
