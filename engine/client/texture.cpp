...
void TextureManager::PrintMemoryUsage() {
    ...
    PLuint mb = texels * ((PLuint) g_video.bpp / 8) / 0x100000;
    Con_Printf("%i textures %i pixels %1.1f megabytes\n", _textures.size(), texels, mb);
}

PLbool TextureManager::IsValidSize(PLuint width, PLuint height) {
...
XTexture *TextureManager::CreateTexture(std::string path, PLuint flags) {
    {
    ...
    if (f) {
        ...
        if (data) {
            _textures[path] = CreateTexture(path, width, height, VL_TEXTUREFORMAT_RGBA8, data, (width * height * 4),
                                            flags);
            free(data);
            return _textures[path];
        }
...
XTexture *
TextureManager::CreateTexture(std::string path, PLuint width, PLuint height, PLTextureFormat format, PLbyte *data,
                              PLuint size, PLuint flags) {
    PLImage image;
    memset(&image, 0, sizeof(PLImage));
    image.data = data;
    image.format = format;
...
XTexture *TextureManager::CreateTexture(PLImage *image, PLuint flags) {
...
void TextureManager::DeleteTexture(XTexture *texture, PLbool force) {
