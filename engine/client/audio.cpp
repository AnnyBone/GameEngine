...
AudioSound *AudioManager::AddSound() {
    AudioSound *sound = new AudioSound;
    memset(sound, 0, sizeof(AudioSound));
...
void AudioManager::SetSoundPosition(AudioSound *sound, plVector3f_t position) {
...
void AudioManager::SetSoundVelocity(AudioSound *sound, plVector3f_t velocity) {
...
void AudioManager::PlaySound(const AudioSound *sound) {
...
void AudioManager::LoadSound(AudioSound *sound, std::string path) {
    if (path[0] == ' ') {
        Con_Warning("Invalid path for sound!\n");
        return;
    } else if (!sound)
...
void AudioManager::StopSound(const AudioSound *sound) {
...
void AudioManager::PauseSound(const AudioSound *sound) {
    if (!sound || !IsSoundPlaying(sound))
        return;
...
void AudioManager::DeleteSound(AudioSound *sound) {
    if (!sound)
...
void AudioManager::SetEffectReverb(const AudioEffect *effect, AudioEffectReverb mode) {
...
bool AudioManager::IsSoundPlaying(const AudioSound *sound) {
    int state;
...
bool AudioManager::IsSoundPaused(const AudioSound *sample) {
    int state;
...
AudioEffect *AudioManager::AddEffect() {
...
void AudioManager::DeleteEffect(AudioEffect *effect) {
...
AudioSample *AudioManager::AddSample(std::string path) {
    AudioSample *cache = new AudioSample;
...
AudioSample *AudioManager::GetSample(std::string path) {
    if (path[0] == ' ') {
...
void AudioManager::PrecacheSample(const char *path, bool preserve) {
    ...
    AudioSample *cache = GetSample(path);
    if (!cache) {
        cache = new AudioSample;
        memset(cache, 0, sizeof(AudioSample));
...
void AudioManager::LoadSample(AudioSample *cache, const char *path) {
    if (!path || (path[0] == ' ')) {
        Con_Warning("Invalid path for audio sample!\n");
        return;
    }
...
void AudioManager::DeleteSample(AudioSample *sample, bool force) {
...
void Audio_PlayAmbientSound(plVector3f_t position, const char *path, float volume) {
    AudioSound *sound = g_audiomanager->AddSound();
    sound->volume = volume;
...
void Audio_StopSound(const AudioSound *sample) {
