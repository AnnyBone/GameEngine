...
bool ModelU3D_Load(model_t *model) {
    ...
    U3DVertex_t *uvertices = (U3DVertex_t *) calloc_or_die((model->numvertexes * model->numframes),
                                                           sizeof(U3DVertex_t));
    for (int i = 0; i < model->numframes; i++) {
        ...
        for (unsigned int j = 0, k = 0; j < model->numtriangles; j++) {
            model->objects[i]->indices[k] = utriangles[j].vertex[0];
            k++;
            model->objects[i]->indices[k] = utriangles[j].vertex[1];
            k++;
            model->objects[i]->indices[k] = utriangles[j].vertex[2];
            k++;
