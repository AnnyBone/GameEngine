...
ConsoleVariable_t
	cvInputMouseLook = {"input_mouselook", "1", true, false,
						"Enables and disables the ability to use the mouse to look around."},
	cvInputMouseFilter = {"input_mousefilter", "0", true, false,
	...
	switch (sEvent.type) {
	  ...
	  case SDL_WINDOWEVENT:
		switch (sEvent.window.event) {
		  default: break;
		  case SDL_WINDOWEVENT_FOCUS_GAINED: Video.bActive = true;
			...
			Key_ClearStates();
			break;
		  case SDL_WINDOWEVENT_FOCUS_LOST: Video.bActive = false;
			...
			Key_ClearStates();
			break;
		  case SDL_WINDOWEVENT_RESIZED: Window_Update();
			break;
		  case SDL_WINDOWEVENT_CLOSE: Sys_Quit();
			break;
		}
		break;
	  case SDL_KEYDOWN:
	  case SDL_KEYUP: Key_Event(Input_ConvertKey(sEvent.key.keysym.sym), (sEvent.key.state == SDL_PRESSED));
		break;
