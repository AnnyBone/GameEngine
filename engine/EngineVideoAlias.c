...
void Alias_SetupLighting(ClientEntity_t *entity) {
    ...
    if (!(entity->effects & EF_FULLBRIGHT)) {
        ...
        if (dlight) {
            ...
            Math_VectorSubtractValue(alias_lightcolour, (dlight->radius - plLengthf(distance)) / 1000.0f,
                                     alias_lightcolour);
    ...
    if (entity > cl_entities && entity <= cl_entities + cl.maxclients) {
        ...
        if (mod > 0.0f) Math_VectorAddValue(alias_lightcolour, mod / 3.0f, alias_lightcolour);
    }
#else
...
void Alias_DrawFrame(MD2_t *alias, ClientEntity_t *entity, lerpdata_t lLerpData) {
    ...
    MD2Frame_t *frame1 = (MD2Frame_t *) ((uint8_t *) alias + alias->ofs_frames +
                                         (alias->framesize * entity->draw_lastpose));
    MD2Frame_t *frame2 = (MD2Frame_t *) ((uint8_t *) alias + alias->ofs_frames +
                                         (alias->framesize * entity->draw_pose));
    ...
    for (;;) {
        ...
        do {
            ...
            Video_ObjectColour(&vertices[uiVerts], alias_lightcolour[0], alias_lightcolour[1], alias_lightcolour[2],
                               fAlpha);
...
void Alias_SetupEntityTransform(ClientEntity_t *ceEntity, lerpdata_t *lerpdata) {
    ...
    } else if (!plVectorCompare(ceEntity->origin, ceEntity->currentorigin) ||
               !plVectorCompare(ceEntity->angles, ceEntity->currentangles)) // origin/angles changed, start new lerp
    {
    ...
    if (r_lerpmove.value && ceEntity != &cl.viewent && ceEntity->lerpflags & LERP_MOVESTEP) {
        if (ceEntity->lerpflags & LERP_FINISH)
            blend = plClamp(0, (cl.time - ceEntity->movelerpstart) / (ceEntity->lerpfinish - ceEntity->movelerpstart),
                            1);
        else
