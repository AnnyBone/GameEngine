...
void _Material_SetType(Material_t *mCurrentMaterial, MaterialContext_t context, char *cArg) {
  switch (context) {
	case MATERIAL_CONTEXT_SKIN: {
	  ...
	  break;
	case MATERIAL_CONTEXT_TEXTURE: _Material_SetTextureType(mCurrentMaterial, context, cArg);
	  break;
	default: Sys_Error("Invalid context!\n");
  }
}
  ...
  if (cToken[0] == '{') {
	while (true) {
	  ...
	  if (cToken[0] == '}') {
		...
		switch (var->type) {
		  case PL_UNIFORM_DOUBLE: program->SetUniformVariable(var, std::strtod(cToken, NULL));
			break;
		  case VL_UNIFORM_FLOAT: program->SetUniformVariable(var, std::strtof(cToken, NULL));
			break;
		  ...
		  case VL_UNIFORM_INT: program->SetUniformVariable(var, std::atoi(cToken));
			break;
  ...
  for (int i = 0; i < plArrayElements(material_types); i++)
	if (strstr(arg, material_types[i].name)) {
	  ...
	  switch (curtex->mttType) {
		case MATERIAL_TEXTURE_DETAIL: curtex->env_mode = PL_TEXTUREMODE_COMBINE;
		  curtex->scale = cv_video_detailscale.value;
		  break;
		case MATERIAL_TEXTURE_FULLBRIGHT: curtex->env_mode = PL_TEXTUREMODE_ADD;
		  break;
		case MATERIAL_TEXTURE_SPHERE: curtex->env_mode = PL_TEXTUREMODE_COMBINE;
		  break;
		default:break;
	  }
	  break;
...
void _Material_SetRotate(Material_t *mCurrentMaterial, MaterialContext_t context, char *cArg) {
  if (context != MATERIAL_CONTEXT_TEXTURE) return;
  ...
  if (showwireframe) {
	...
	if (!ispost) {
	  switch (object->primitive) {
		case VL_PRIMITIVE_LINES: break;
		case VL_PRIMITIVE_TRIANGLES: object->primitive_restore = VL_PRIMITIVE_TRIANGLES;
...
*/
void Material_Draw(Material_t *material,
