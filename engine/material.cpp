...
Material g_materials[MATERIAL_MAX];    // Global array.
...
// Global Materials
Material *g_mHDAccess;
Material *g_mMissingMaterial;
Material *g_mBlobShadow;
Material *g_mGlobalColour;
Material *g_mGlobalConChars;
...
Material *Material_Allocate(void) {
    ...
    memset(&g_materials[material_count], 0, sizeof(Material));
...
*/
void Material_ClearSkin(Material *material, unsigned int skinnum) {
    MaterialSkin *skin = Material_GetSkin(material, skinnum);
    if (!skin)
...
MaterialSkin *Material_GetSkin(Material *mMaterial, unsigned int iSkin) {
...
*/
MaterialSkin *Material_GetAnimatedSkin(Material *mMaterial) {
...
*/
Material *Material_Get(int iMaterialID) {
    int i;
...
/*	Returns true on success.
	Unfinished
*/
Material *Material_GetByName(const char *ccMaterialName) {
    if (ccMaterialName[0] == ' ') {
...
Material *Material_GetByPath(const char *ccPath) {
    if (ccPath[0] == ' ') {
...
void Material_CheckFunctions(Material *mNewMaterial);
...
void _Material_SetTextureType(Material *mCurrentMaterial, MaterialContext_t mftContext, char *cArg);

void _Material_SetType(Material *mCurrentMaterial, MaterialContext_t context, char *cArg) {
    switch (context) {
...
void _Material_SetWireframe(Material *mCurrentMaterial, MaterialContext_t context, char *cArg) {
    if (context != MATERIAL_CONTEXT_GLOBAL) return;
    mCurrentMaterial->override_wireframe = (bool) atoi(cArg);
}

void _Material_SetLightmap(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_GLOBAL) return;
    material->override_lightmap = (bool) atoi(arg);
}

void _Material_SetAlpha(Material *mCurrentMaterial, MaterialContext_t context, char *cArg) {
    if (context != MATERIAL_CONTEXT_GLOBAL) return;
    mCurrentMaterial->fAlpha = strtof(cArg, NULL);
}

void _Material_SetAnimationSpeed(Material *mCurrentMaterial, MaterialContext_t context, char *cArg) {
    if (context != MATERIAL_CONTEXT_GLOBAL) return;
...
void _Material_ParseSkin(Material *material, MaterialContext_t context, char *args) {
    if (context != MATERIAL_CONTEXT_GLOBAL) return;

    MaterialSkin *skin = Material_GetSkin(material, material->num_skins);
    ...
    if (cToken[0] == '{') {
        while (true) {
            ...
            if (cToken[0] == '}') {
                ...
                pl::graphics::ShaderUniform *var = program->GetUniform(cToken + 1);
                if (!var) {
                ...
                switch (var->type) {
                    ...
                    case pl::graphics::UNIFORM_VEC2: {
                        PLVector2D vec;
                        if (sscanf(cToken, "%f %f", &vec.x, &vec.y) < 2) {
                    ...
                    case pl::graphics::UNIFORM_VEC3: {
                        PLVector3D vec;
                        if (sscanf(cToken, "%f %f %f", &vec.x, &vec.y, &vec.z) < 3) {
                            Con_Warning("Field did not return expected number of arguments! (%s) (%i)\n", cToken,
                                        iScriptLine);
                        }
                    ...
                    default: Con_Warning("Unsupported or invalid data type! (%s) (%i)\n", cToken, iScriptLine);
                }
...
void _Material_AddTexture(Material *material, MaterialContext_t context, char *cArg) {
    if (context != MATERIAL_CONTEXT_SKIN) return;

    MaterialSkin *curskin = Material_GetSkin(material, material->num_skins);
    if (!curskin) {
...
#	pragma warning(suppress: 6011)
#endif
    MaterialTexture *curtexture = &curskin->texture[curskin->num_textures];
    memset(curtexture, 0, sizeof(MaterialTexture));
...
void _Material_SetTextureType(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_TEXTURE) return;

    MaterialSkin *curskin = Material_GetSkin(material, material->num_skins);
    MaterialTexture *curtex = &curskin->texture[curskin->num_textures];
...
void _Material_SetTextureScroll(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_TEXTURE) return;

    PLVector2D scroll(0, 0);
    if (sscanf(arg, "%f %f", &scroll.x, &scroll.y) != 2) {
        Sys_Error("Invalid arguments! (%s) (%i)", material->cPath, iScriptLine);
    }

    MaterialSkin *curskin = Material_GetSkin(material, material->num_skins);
    curskin->texture[curskin->num_textures].scroll.x = scroll.x;
    curskin->texture[curskin->num_textures].scroll.y = scroll.y;
...
void _Material_SetTextureScale(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_TEXTURE) return;
...
void _Material_SetTextureEnvironmentMode(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_TEXTURE) return;

    MaterialSkin *sCurrentSkin = Material_GetSkin(material, material->num_skins);
...
void _Material_SetRotate(Material *mCurrentMaterial, MaterialContext_t context, char *cArg) {
    if (context != MATERIAL_CONTEXT_TEXTURE) return;

    // Get the current skin.
    MaterialSkin *msSkin = Material_GetSkin(mCurrentMaterial, mCurrentMaterial->num_skins);
...
void _Material_SetAdditive(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_SKIN) return;
...
void _Material_SetBlend(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_SKIN) return;
...
void _Material_SetAlphaTest(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_SKIN) return;
...
void _Material_SetAlphaTrick(Material *material, MaterialContext_t context, char *arg) {
    if (context != MATERIAL_CONTEXT_SKIN) return;
...
*/
void _Material_SetFlags(Material *mCurrentMaterial, MaterialContext_t mftContext, char *cArg) {
    // Search through and copy each flag into the materials list of flags.
            ...
            switch (mftContext) {
                ...
                default:
                    Con_Warning("Invalid context! (%s) (%s) (%i) (%i)\n",
                                mCurrentMaterial->cName,
                                mfMaterialFlags[i].ccName,
                                mftContext,
                                iScriptLine);
            }
            return;
...
typedef struct {
    ...
    void (*Function)(Material *mCurrentMaterial, MaterialContext_t mftContext, char *cArg);
...
typedef struct
{
    ...
    void					(*Function)(Material *material, MaterialContext_t context, char *arg);
} MaterialFunction_t;
...
*/
void Material_ParseFunction(Material *material)
{
    // Find the related function.
...
*/
Material *Material_Load(const char *ccPath) {
    void *cData;
    char cPath[PL_SYSTEM_MAX_PATH], cMaterialName[64] = {0};
    ...
    Material *mNewMaterial = Material_GetByPath(cPath);
    if (mNewMaterial)
        return mNewMaterial;
...
bool Material_Precache(const char *path) {
    Material *mNewMaterial = Material_Load(path);
    if (!mNewMaterial) {
    ...
    if (showwireframe) {
        ...
        if (!ispost) {
            switch (object->primitive) {
                case PL_PRIMITIVE_LINES:
                    break;
                case PL_PRIMITIVE_TRIANGLES:
                    object->primitive_restore = PL_PRIMITIVE_TRIANGLES;
                    object->primitive = PL_PRIMITIVE_LINES;
                    break;
                default:
                    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
            }
        } else {
            if ((object->primitive != PL_PRIMITIVE_LINES) &&
                (object->primitive != PL_PRIMITIVE_TRIANGLES))
                glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
            else if (object->primitive == PL_PRIMITIVE_TRIANGLES)
...
void Material_Draw(Material *material,
    ...
    if (!material) return; // todo, handle this better... throw error?
        ...
        MaterialSkin *skin = Material_GetSkin(material, material->current_skin);
        if (!skin) {
            Sys_Error("Failed to get valid skin! (%s)\n", material->cName);
            return;
        } // return to keep compiler happy...

        MaterialTexture *texture = &skin->texture[0];
        if (!texture) {
    ...
    for (unsigned int unit = 0, i = 0; i < msCurrentSkin->num_textures; i++, texture++, unit++) {
        ...
        if (!ispost) {
            ...
            if (texture->matrixmod) {
                ...
                if ((texture->scroll.x > 0) || (texture->scroll.x < 0) ||
                    (texture->scroll.y > 0) || (texture->scroll.y < 0)) {
                    glTranslatef(
                            texture->scroll.x * (float) cl.time,
                            texture->scroll.y * (float) cl.time,
                            0);
                }
                    ...
                    for (unsigned int j = 0; j < ObjectSize; j++) {
                        ...
                        Video_ObjectTexture(&ObjectVertex[j], unit,
                                // Use base texture coordinates as a reference.
                                            ObjectVertex[j].st[0].x * texture->scale,
                                            ObjectVertex[j].st[0].y * texture->scale);
        ...
        switch (texture->mttType) {
            ...
            case MATERIAL_TEXTURE_SPHERE:
                if (!ispost) {
                    ...
                    plEnableGraphicsStates(PL_CAPABILITY_TEXTURE_GEN_S | PL_CAPABILITY_TEXTURE_GEN_T);
                } else
                    plDisableGraphicsStates(PL_CAPABILITY_TEXTURE_GEN_S | PL_CAPABILITY_TEXTURE_GEN_T);
                break;
            default:
                break;
        }
