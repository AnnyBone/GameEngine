...
extern "C" unsigned blocklights[BLOCK_WIDTH * BLOCK_HEIGHT *
                                3]; //johnfitz -- was 18*18, added lit support (*3) and loosened surface extents maximum (BLOCK_WIDTH*BLOCK_HEIGHT)
...
void Brush_Draw(ClientEntity_t *e) {
    ...
    if (clmodel->firstmodelsurface != 0 && !gl_flashblend.value) {
        ...
        for (k = 0; k < MAX_DLIGHTS; k++) {
            if (!cl_dlights[k].lightmap || ((cl_dlights[k].die < cl.time) && cl_dlights[k].die))
                continue;
...
void R_BuildLightMap(msurface_t *surf, byte *dest, int stride) {
    ...
    if (cl.worldmodel->lightdata) {
        ...
        if (lightmap)
            for (maps = 0; maps < BSP_MAX_LIGHTMAPS && surf->styles[maps] != 255; maps++) {
                scale = (unsigned int)d_lightstylevalue[surf->styles[maps]];
    ...
    for (i = 0; i < tmax; i++, dest += stride) {
        for (j = 0; j < smax; j++, dest += 4) {
            t = *bl++ >> cv_video_lightmapoversample.iValue;
            if (t > 255) t = 255;
            dest[2] = t;
            t = *bl++ >> cv_video_lightmapoversample.iValue;
            if (t > 255) t = 255;
