...
#endif
	default: Sys_Error("Unknown texture environment mode! (%i)\n", TextureEnvironmentMode);
  }
...
void vlRenderBufferStorage(int format, int samples, unsigned int width, unsigned int height) {
  ...
  if (glerror != GL_NO_ERROR) {
	char *errorstring = "";
	switch (glerror) {
	  case GL_INVALID_ENUM: errorstring = "Invalid internal format!";
		break;
	  case GL_OUT_OF_MEMORY: errorstring = "Unable to create a data store of the requested size!";
		break;
	  default:break;
	}
...
void vlCheckFrameBufferStatus(PLFBOTarget target) {
  ...
  switch (status) {
	case GL_FRAMEBUFFER_UNDEFINED: Sys_Error("Default framebuffer doesn't exist!\n");
	case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: Sys_Error("Framebuffer attachment points are incomplete!\n");
	case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: Sys_Error("Framebuffer doesn't have an image attached!\n");
	case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: Sys_Error("Invalid attachment type!\n");
	case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER: Sys_Error("INCOMPLETE READ BUFFER\n");
	case GL_FRAMEBUFFER_UNSUPPORTED: Sys_Error("UNSUPPORTED\n");
	case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE: Sys_Error("INCOMPLETE MULTISAMPLE\n");
	case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_ARB: Sys_Error("INCOMPLETE LAYER TARGETS\n");
  }
...
void vlBindFrameBuffer(PLFBOTarget target, unsigned int buffer) {
  ...
  if (glerror != GL_NO_ERROR) {
	char *errorstring = "";
	switch (glerror) {
	  case GL_INVALID_ENUM: errorstring = "Invalid framebuffer target!";
		break;
	  case GL_INVALID_OPERATION: errorstring = "Invalid framebuffer object!";
		break;
	  default:
...
void vlEndDraw(PLDraw *draw) {
  ...
  glBufferData(GL_ARRAY_BUFFER,
			   draw->numverts * sizeof(plVector3f_t) * sizeof(PLVertex),
			   draw->vertices->position,
			   GL_DYNAMIC_DRAW);

  glBindBuffer(GL_ARRAY_BUFFER, draw->_gl_vbo[_VL_BUFFER_TEXCOORDS]);
  glBufferData(GL_ARRAY_BUFFER,
			   draw->numverts * sizeof(plVector2f_t) * sizeof(PLVertex),
			   draw->vertices->ST,
			   GL_DYNAMIC_DRAW);
