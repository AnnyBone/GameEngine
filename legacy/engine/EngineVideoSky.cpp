...
void Sky_ProjectPoly(int nump, MathVector3f_t vecs) {
    ...
    for (i = 0, vp = vecs; i < nump; i++, vp += 3) Math_VectorAdd (vp, v, v);
...
void Sky_ProcessPoly(glpoly_t *p) {
    ...
    // Update sky bounds
    if (!r_fastsky.value) {
        for (i = 0; i < p->numverts; i++) Math_VectorSubtract (p->verts[i], camera->GetPosition(), verts[i]);
            ...
            if (s->flags & SURF_DRAWSKY) {
                ...
                if (((s->flags & SURF_PLANEBACK) && (dot < -BACKFACE_EPSILON)) ||
                    ...
                    for (k = 0; k < p->numverts; k++) {
                        ...
                        } else Math_VectorAdd(s->polys->verts[k], e->origin, p->verts[k]);
                    }
                    Sky_ProcessPoly(p);
                    Hunk_FreeToLowMark(mark);
                }
