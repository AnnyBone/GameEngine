...
bool ModelU3D_Load(model_t *model) {
    ...
    model->objects = (PLDraw **) calloc_or_die((size_t)model->numframes, sizeof(*model->objects));
    ...
    U3DVertex_t *uvertices = (U3DVertex_t *) calloc_or_die((size_t) (model->numvertexes * model->numframes),
                                                           sizeof(U3DVertex_t));
    for (int i = 0; i < model->numframes; i++) {
        ...
        for (int j = 0; j < model->numvertexes; j++) {
            vlDrawColour4fv(PLColour(PL_COLOUR_WHITE));
