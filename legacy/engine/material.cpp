...
Material_t *Material_Load(const char *ccPath) {
    ...
    std::string fullpath = g_state.path_materials + std::string(ccPath) + ".material";

    // Check if it's been cached already...
    Material_t *mNewMaterial = Material_GetByPath(fullpath.c_str());
    if (mNewMaterial)
        return mNewMaterial;

    cData = COM_LoadHeapFile(fullpath.c_str());
    if (!cData) {
        Con_Warning("Failed to load material! (%s) (%s)\n", fullpath.c_str(), ccPath);
        return NULL;
    }
    ...
    if (!Script_GetToken(true)) {
        ...
        material_name = cToken;
        if (material_name.empty())
            Sys_Error("Invalid material name!\n");

        // Check if it's been cached already...
        mNewMaterial = Material_GetByName(material_name.c_str());
        if (mNewMaterial) {
            Con_Warning("Attempted to load duplicate material! (%s) (%s) vs (%s) (%s)\n",
                        ccPath, material_name.c_str(),
    ...
    if (!material_name.empty())
        strncpy(mNewMaterial->cName, material_name.c_str(), sizeof(mNewMaterial->cName));
    else {
    ...
    if (ispost) {
        ...
        if ((msCurrentSkin->uiFlags & MATERIAL_FLAG_BLEND) || (material->fAlpha < 1)) {
            ...
            if ((msCurrentSkin->uiFlags & MATERIAL_FLAG_ALPHATRICK) &&
                (cv_video_alphatrick.bValue && (ObjectSize > 0))) {
                vlDepthMask(false);
                plEnableGraphicsStates(PL_CAPABILITY_BLEND);
