...
void draw::SetDefaultState() {
    plSetClearColour(PLColour(150, 0, 0, 255));
...
void draw::BoundingBoxes() {
    ...
    plDisableGraphicsStates(VL_CAPABILITY_DEPTH_TEST | PL_CAPABILITY_TEXTURE_2D);
    ...
    plEnableGraphicsStates(PL_CAPABILITY_TEXTURE_2D | VL_CAPABILITY_DEPTH_TEST);
}
...
void draw::Rectangle(PLint x, PLint y, PLuint w, PLuint h, PLColour colour) {
    ...
    if (colour[3] < 1)
        plDisableGraphicsStates(PL_CAPABILITY_BLEND);
    plEnableGraphicsStates(VL_CAPABILITY_DEPTH_TEST | PL_CAPABILITY_TEXTURE_2D);
}
