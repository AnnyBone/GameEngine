...
/* Particle System
 * TODO: Stub everything here and rewrite it.
 */
...
ConsoleVariable_t cv_video_drawparticles = {"video_particles", "1", true, false,
                                            "Toggles the rendering of particles."};    // TODO: Move into enginevideo!
...
Particle_t *Particle_Allocate(void) {
    ...
    plVectorSet3f(pParticle->vColour, 1, 1, 1);
...
void ParticleManager_Simulate(void) {
    ...
    fFrameTime = (float) (cl.time - cl.oldtime);
    ...
    for (pParticle = pActiveParticles; pParticle; pParticle = pParticle->next) {
        ...
        pParticle->vColour[PL_ALPHA] = (PLbyte) (plClamp(0, pParticle->lifetime - cl.time, 1.0f) / 255);
